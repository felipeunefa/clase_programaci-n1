...
class Detalle_de_factura extends Conexion {
    ...
    public function get_select(){
        $result = $this->_db->query('select * from detalle_de_factura where estado=1'); 
        ...
        foreach ($detalleData as $detalle_de_factura):
            $select=$select. '<option value="'.$detalle_de_factura['detalle de factura'].'">'.$detalle_de_factura['nit'].'</option>';
        endforeach;
